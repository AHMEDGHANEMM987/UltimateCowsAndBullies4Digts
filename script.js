...
Ahmed Ghanem, [10/05/2025 3:43 AM]
body {
...
Ahmed Ghanem, [10/05/2025 3:45 AM]
// توليد الرقم السري العشوائي
let secretNumber = Math.floor(Math.random() * 9000) + 1000;  // رقم مكون من 4 أرقام عشوائية
let attempts = 0;
...
function checkGuess() {
    ...
    if (guess.length !== 4 || isNaN(guess)) {
        document.getElementById("message").innerText = "Please enter a 4-digit number!";
        return;
    }
    ...
    // مقارنة الأرقام
    for (let i = 0; i < 4; i++) {
        if (guess[i] === secretStr[i]) {
            taureaux++;
        } else if (secretStr.includes(guess[i])) {
            vaches++;
        }
    }

    // عرض الرسائل بناءً على النتيجة
    if (taureaux === 4) {
        document.getElementById("message").innerText = "Congratulations! You guessed the secret number!";
        document.getElementById("attempts").innerText = It took you ${attempts} attempts.;
    } else {
        document.getElementById("message").innerText = ${taureaux} Taureaux, ${vaches} Vaches;
